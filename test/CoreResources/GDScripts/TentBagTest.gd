...
func test_use(timeout=10):
	...
	assert_object(world_placements.get_child(1)).is_instanceof(Tent)
	assert_object(tent_bag.hannah.inventory.get_node("%InvSprite").get_node("%LooseItems").get_child(0)).is_instanceof(BaseItem)
	assert_object(tent_bag.hannah.inventory.get_node("%InvSprite").get_node("%LooseItems").get_child(1)).is_instanceof(BaseItem)
